...
const createTweet = catchAsync(async (req, res) => {
  const tweet = await tweetService.createTweet({...req.body, user: req.user.id});
  res.status(httpStatus.CREATED).send({ tweet });
});

const updateTweet = catchAsync(async (req, res) => {
    const tweet = await tweetService.updateTweet(req.params.tweetId, req.user.id, {...req.body});
